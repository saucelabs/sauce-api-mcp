...
class AlwaysMatch(BaseModel):
    goog_chromeOptions: GoogChromeOptionsRequest = Field(..., alias='goog:chromeOptions')
    browserName: str
...
class ResultCapabilities(BaseModel):
    ...
    browserVersion: str
    webauthn_extension_minPinLength: bool = Field(..., alias='webauthn:extension:minPinLength')
    timeouts: Timeouts
    ...
    proxy: Dict[str, str]
    webauthn_virtualAuthenticators: bool = Field(..., alias='webauthn:virtualAuthenticators')
    pageLoadStrategy: str
    webauthn_extension_largeBlob: bool = Field(..., alias='webauthn:extension:largeBlob')
    platformName: str
...
class TrendsAggregations(BaseModel):
    ...
    device_passed: List[NameCount] = Field(..., alias='devicePassed')
...
class Metrics(BaseModel):
    """
    Represents the overall aggregated metrics across all buckets.
    """
    ...
    owner: Dict[str, int]
    status: Dict[str, int]
    browser_fail: Dict[str, int] = Field(..., alias='browserFail')
    device_passed: Dict[str, int] = Field(..., alias='devicePassed')
    device_fail: Dict[str, int] = Field(..., alias='deviceFail')
    framework_fail: Dict[str, int] = Field(..., alias='frameworkFail')
    os_fail: Dict[str, int] = Field(..., alias='osFail')
...
class TestTrends(BaseModel):
    """
    The main model for the analytics report response.
    """
    meta: Meta
    buckets: List[Bucket]
    metrics: Metrics
